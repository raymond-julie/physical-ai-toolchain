...
ROS_PACKAGES=(
    ...
    # cv_bridge is required by UrRecorder/leader_follower/lerobot_recorder_node.py to convert
...
# Intel RealSense ROS 2 driver, used by UrRecorder/leader_follower/run_recorder.sh to
# publish /camera/camera/color/image_raw and /camera/camera/depth/image_rect_raw.
...
# librealsense apt repo (see HOW_TO_USE.md troubleshooting). We do not let a
...
# LeRobot dataset writer used by UrRecorder/leader_follower/lerobot_recorder_node.py.
...
Next steps:
  ...
  cd ${SCRIPT_DIR}
  colcon build --packages-select ur_status_copy
