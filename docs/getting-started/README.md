...
Adoption is modeled as six graduated tiers (T0–T5). Each tier states the minimum infrastructure to reach **Goal: Full Training Lifecycle** — capture demonstrations on a robot, train an imitation policy, validate it, and run that policy back on the robot — and **each tier is a legitimate stopping point**. Start at T0 and graduate when (and only when) a concrete trigger forces it.

| Tier                | When to start here                                                   | Graduate when…                                                                                                                                             | Quick start                                                   |
|---------------------|----------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------|
| **T0 — Dev** ⭐      | Default. One laptop, one robot — zero cloud, zero Kubernetes.        | No local GPU; the task needs many training iterations as conditions vary; or a second person needs the data.                                               | [Tier 0 — Dev](../recipes/tier-0-dev/README.md)               |
...
| **T2 — Pilot** ✅    | Recommended production. One site at scale; cloud training default.   | The robot count or update cadence makes hand-updating each robot error-prone and version skew real — still one reachable site.                             | [Tier 2 — Pilot](../recipes/tier-2-pilot/README.md)           |
...
| **T4 — Scale**      | Advanced. Multi-site **fleet delivery**; Arc reachability broker.    | You explicitly want production signals to drive retraining and fleet-wide health analytics — a deliberate decision, not an automatic consequence of scale. | [Tier 4 — Scale](../recipes/tier-4-scale/README.md)           |
| **T5 — Operate**    | Roadmap. **Fleet intelligence** — drift detection and retraining.    | (Roadmap direction — mostly unimplemented today.)                                                                                                          | [Tier 5 — Operate](../recipes/tier-5-operate/README.md)       |
...
> [!NOTE]
> **Roadmap honesty.** T5 (Operate / fleet intelligence) is a roadmap direction, not shipped capability — the `fleet-intelligence` domain ships ~0 Python files and a handful of placeholder specs.

### Start Here — T0 Dev

The default starting path: **one laptop and one robot**, with zero cloud and zero Kubernetes. You close the full capture → train → validate → run loop entirely on local hardware.
...
| [Quickstart](quickstart.md)                | Cloud path (T2 — Pilot): clone to first cloud training job |
...
The default path (T0 — Dev) needs only **Python ≥3.12** plus ROS 2 and Docker — no Azure subscription, no Kubernetes tooling. The additional tools below are required only for the cloud path ([Quickstart](quickstart.md), T2 — Pilot and up).
